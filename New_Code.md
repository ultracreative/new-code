...
The compiler in v1.0 is a real LLM call when online — an Anthropic API call wrapped in AST-level safety checks and a constraint report against the declared intent / forbid / ensure clauses. Accepted bodies are written to a content-addressable snapshot store, so yesterday's online run becomes today's deterministic offline replay. There is nothing pretended about this layer. It is the implementation the specification asked for.

New Code is also not a DSL for signal processing. The 𝕎 primitive looks audio-specific because audio is the domain where four-component waveform structure is already native vocabulary, but the abstraction generalises: a UI state, a belief, a transaction, a database stream — all have frequency, amplitude, phase, and shape as latent components. What makes New Code worth building is the claim that this structure is universal, that intent is a first-class declaration, and that the combination kills categories of bug that conventional languages cannot name.
